...
void Game::OnKeyDown(unsigned char key) {
    ...
    switch (key) {
    case 9: // [추가] TAB 키 (ASCII 9) : 메뉴 카테고리 변경
        if (ui.menuOpen) {
            ui.NextCategory();
        }
        break;
    ...
    case 13:  // [수정] Enter키 - 선택 확정 (공 + 텍스처)
        if (ui.menuOpen) {
            ui.SelectItem(); // SelectBall -> SelectItem으로 변경됨
        }
        break;
