...
public:
    ...
    // [2번 추가] 메뉴 카테고리 (0: Ball, 1:Lane, 2:Wall, 3:Ceiling, 4:Floor)
    int menuCategory;

    // [2번 추가] 각 카테고리별 미리보기 인덱스
    int previewLaneTex;
    int previewWallTex;
    ...
    void DrawTexturePreview(GLuint texID, float x, float y, float size);  // [2번] 텍스처 미리보기

    // 메뉴 조작
    void ToggleMenu();
    void NextCategory();        // [2번] Q키로 카테고리 전환 (Tab 대체)
    void MenuLeft();            // 방향키 왼쪽 (아이템 변경)
    void MenuRight();           // 방향키 오른쪽 (아이템 변경)
    void SelectBall();          // [1번 호환] 공만 선택
    void SelectItem();          // [2번] 모든 선택 확정 (공 + 텍스처)
