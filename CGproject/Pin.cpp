...
vec2 Pin::GetProfilePoint(float t) {
    ...
    float radius = EvaluateCardinalSpline1D(
        controlRadius[i0], controlRadius[i1],
        controlRadius[i2], controlRadius[i3], localT
    );
...
void Pin::GenerateRevolutionMesh() {
    ...
    for (int i = 0; i <= heightSegments; i++) {
        float t = (float)i / (float)heightSegments * 0.90f;
...
void Pin::Update(float dt) {
    // ✅ 게임에서 제거된 핀은 업데이트 안 함
    if (!inPlay) return;

    // ✅ 쓰러진 핀은 즉시 레인에서 제거 (1번 코드 기능)
...
void Pin::ApplyImpact(vec3 impactDir, float force) {
    isFalling = true;
    velocity = impactDir * force * 0.5f;
    velocity.y = force * 0.15f;
    angularVelocity.x = -impactDir.z * force * 0.8f;
    angularVelocity.z = impactDir.x * force * 0.8f;
}
...
bool Pin::IsDown() {
    return !isStanding || abs(rotation.x) > 70.0f || abs(rotation.z) > 70.0f;
}
...
void PinManager::CheckBallCollision(vec3 ballPos, float ballRadius, vec3& ballVelocity, vec3 ballAngularVelocity) {
    ...
    // ✅ 포켓 스트라이크 확률 (1번:  20%, 2번: 5%) → 15%로 절충
    Pin& head = pins[0];
