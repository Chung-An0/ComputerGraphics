...
void UI::Reset() {
    ...
    // [2번] 메뉴 카테고리 및 미리보기 초기화
    menuCategory = 0; // 0: 공 선택 메뉴에서 시작
    previewLaneTex = 0;
    previewWallTex = 0;
    previewCeilingTex = 0;
    previewFloorTex = 0;
...
void UI::DrawMenu() {
    ...
    DrawTextCentered(WINDOW_WIDTH / 2, menuY + 50, "Q/TAB: Change Category");
...
void UI::ToggleMenu() {
    menuOpen = !menuOpen;
    if (menuOpen) {
        // [2번] 메뉴 열 때 현재 선택 상태를 UI 변수로 가져오기 (동기화)
